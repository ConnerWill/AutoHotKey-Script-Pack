...
;   ==========================   GUI Text/Controls Design	==========================



; 	-------------- Top Text -------------- 
;   -----
TopTextText = VPN Configuration
TopTextSize = 12
TopTextColor = FFFFFF 
TopTextProperties = Bold underline
TopTextFontFamily = MS Shell Dlg 2
;   -----
...
BackgroundColor = EEAA99

BackgroundTransparentColor = 000000
...
WinSet, TransColor, %BackgroundTransparentColor%
WinSet,AlwaysOnTop

;~ Gui -Caption  ; remove the border and title bar from a window with a transparent background, use the following after the window has been made transparent Or use Gui, GuiName:-Caption if it isn't the default window.

;~ iconsize := 32  ; Ideal size for alt-tab varies between systems and OS versions.
;~ hIcon := LoadPicture("C:\Users\cwill\Pictures\Icons\AppDeployToolkitLogo.ico", "Icon1 w" iconsize " h" iconsize, imgtype)
;~ Gui +LastFound
;~ SendMessage 0x0080, 1, hIcon  ; 0x0080 is WM_SETICON; and 1 means ICON_BIG (vs. 0 for ICON_SMALL).
;~ Gui Show

;Gui, Font, s%TopTextSize% %TopTextProperties%, %TopTextFontFamily% c%TopTextColor%, 

Gui, Font, s%TopTextSize% bold, MS Shell Dlg 2
Gui, Add, Text, xm ym w195 0x201 c%TopTextColor%, % "CPU Usages"
